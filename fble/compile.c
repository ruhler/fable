...
static void PrintType(FbleArena* arena, Type* type, TypeList* printed)
{
  ...
  switch (type->tag) {
    ...
    case PROC_TYPE: {
      ProcType* pt = (ProcType*)type;
      PrintType(arena, pt->rtype, &nprinted);
      fprintf(stderr, "!");
      break;
    }
...
static Type* CompileExpr(TypeArena* arena, Vars* vars, FbleExpr* expr, FbleInstrV* instrs)
{
  ...
  switch (expr->tag) {
    ...
    case FBLE_STRUCT_VALUE_EXPR: {
      ...
      Type* type = CompileType(arena, vars, struct_value_expr->type);
    ...
    case FBLE_GET_EXPR: {
      ...
      Type* type = CompileExpr(arena, vars, get_expr->port, instrs);
      Eval(arena, type, NULL, NULL);
      if (type == NULL) {
        return NULL;
      }
    ...
    case FBLE_PUT_EXPR: {
      ...
      // Compile the port value.
      Type* type = CompileExpr(arena, vars, put_expr->port, instrs);
      Eval(arena, type, NULL, NULL);
      if (type == NULL) {
        return NULL;
      }
...
static Type* CompileType(TypeArena* arena, Vars* vars, FbleType* type)
{
  ...
  switch (type->tag) {
    ...
    case FBLE_PROC_TYPE_EXPR: {
      ...
      FbleProcTypeExpr* proc_type = (FbleProcTypeExpr*)type;
    ...
    case FBLE_INPUT_TYPE_EXPR: {
      ...
      FbleInputTypeExpr* input_type = (FbleInputTypeExpr*)type;
    ...
    case FBLE_OUTPUT_TYPE_EXPR: {
      ...
      FbleOutputTypeExpr* output_type = (FbleOutputTypeExpr*)type;
