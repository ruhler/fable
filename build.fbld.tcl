...
proc fbld-check-error-run {tloc program loc} {
...
proc fblc-check-error { program loc } {
  ...
  testl $tloc fbld-check-error-run $tloc $program $loc
}
...
proc fbld-test-run { tloc program entry args script} {
  set line [dict get $tloc line]
  set file [dict get $tloc file]
  set name "[file tail $file]_$line"
...
proc fblc-test { program entry args script } {
  set tloc [info frame -1]
  testl $tloc fbld-test-run $tloc $program $entry $args $script
}
...
test exec $::fbldtest prgms/AllTests.wnt prgms/ Test@AllTests
test exec $::fbldcheck prgms/ Snake

exec echo "hello" > out/test/fbld/hello.txt
test exec $::fbldmd5 prgms/ Md5@Md5@Md5 out/test/fbld/hello.txt | grep b1946ac92492d2347c6235b4d2611184
